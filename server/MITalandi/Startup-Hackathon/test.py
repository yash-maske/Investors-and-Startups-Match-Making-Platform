...
def preprocessing(df,column):
    df[column] = df[column].apply(lambda x: str(x).split() if pd.notnull(x) else [])
...
def desc_similarity(df,column1,column2):
    ...
    df_similar_N = df_similar.iloc[0:10, :]  # Top 10

    # Get the original indices from df1
    df_similar_N = df_similar_N.merge(df1.reset_index(), on='company')  # Add index from df1
    df_similar_index = df_similar_N['index'].tolist()
